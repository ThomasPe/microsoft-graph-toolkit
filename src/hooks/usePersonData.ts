  ...
  useEffect(() => {
    if (provider instanceof MockProvider) {
      ...
      const sampleAvatarUrl =
        'https://github.com/microsoftgraph/microsoft-graph-toolkit/raw/main/assets/graff.png';
    ...
    const fetchData = async () => {
      try {
        ...
        if (fetchPresence && user.id) {
          try {
            presence = await graphClient.api(`/users/${user.id}/presence`).get();
          } catch (err) {
        ...
        if (fetchPhoto && user.id) {
          try {
            const photoBlob = await graphClient.api(`/users/${user.id}/photo/$value`).get();
